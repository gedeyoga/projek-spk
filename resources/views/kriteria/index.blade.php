                    ...
                    <div class="col-8 col-md-3 ml-auto">
                        <form action="{{ route('kriteria.index') }}" method="get">
                            <div class="input-group">
                                <input name="search" type="text" class="form-control" placeholder="Cari data kriteria..">
                ...
                <table class="table table-striped mb-3 table-sm">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="d-flex">
                                <button onclick="onShowModal('{{ $kriteria->id }}')" class="btn btn-sm btn-info mr-2"><i class="fas fa-pencil-alt fa-fw mr-1"></i>Edit</button>
                                <button onclick="handleDelete('{{ $kriteria->id }}')" class="btn btn-sm btn-danger mr-2"><i class="fas fa-trash fa-fw mr-1"></i>Hapus</button>
                            </td>
                        </tr>
                        @endforeach
                    ...
                    <div class="col-12 d-flex justify-content-end">
                        <nav aria-label="Page navigation example">
                            <ul class="pagination">
                                ...
                                <li class="page-item"><a class="page-link" href="{{ $kriterias->previousPageUrl() }}">Previous</a></li>
                                @endif

                                @php
                                $start = $kriterias->currentPage() - 2;
                                $start = $start < 1 ? 1 : $start; $end=$kriterias->currentPage() + 2;
                                    $end = $end > $kriterias->lastPage() ? $kriterias->lastPage() : $end;
                                    @endphp

                                    @for( $no = $start; $no <= $end; $no++) <li class="page-item  {{ $kriterias->currentPage() == $no ? 'active' : '' }}">
                                        <a class="page-link" href="{{ route('kriteria.index') . '?page=' . $no }}">{{ $no }}</a>
                                        </li>
                                        @endfor

                                        @if( $kriterias->nextPageUrl())
                                        <li class="page-item"><a class="page-link" href="{{ $kriterias->nextPageUrl() }}">Next</a></li>
                                        @endif
                            </ul>
